...
function rgbatohexa(rgba: any) {
  ...
  let g = (Number(rgba[1])).toString(16)
  let b = (Number(rgba[2])).toString(16)
  let a = Math.round(Number(rgba[3]) * 255).toString(16)
  if (r.length === 1)
    r = '0' + r
  if (g.length === 1)
    g = '0' + g
...
export function converter(color: any) {
  ...
  if (color[0].includes('color')) {
    if (color[1].includes('rgba')) {
      ...
      color[1] = color[1].replace(rgbaVal, hexVal)
      result[color[0]] = color[1]
    } else if (!color[1].includes('!important')) {
  ...
  if (rules.includes(color[0])) {
    ...
    if (color[1].includes('rgba')) {
      ...
      } else if (colourNameToHex(splited[splited.length - 2].trim()) !== false) {
